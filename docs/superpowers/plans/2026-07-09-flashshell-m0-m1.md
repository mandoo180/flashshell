...
- 코어유틸의 출력 형식과 에러 메시지의 기준은 **`debian:stable-slim` 의 GNU coreutils** 다. macOS의 BSD 계열이 아니다. 형식을 추측하지 말고 골든 테스트로 확인한다.
- 파일 크기와 `wc -c` 는 **진짜 바이트 수**를 센다. JS 문자열의 `.length` 는 UTF-16 코드유닛이므로 `'한'.length === 1` 이지만 GNU 는 3을 센다. `byteLength()` 헬퍼를 쓴다.
- 정렬 비교에 `localeCompare` 를 쓰지 않는다. `C` 로케일의 바이트 순(대문자 먼저)이 기준이며 JS의 `<` 가 그것과 일치한다.
...
**Files:**
- Create: `src/shell/types.ts`, `src/shell/registry.ts`, `src/shell/builtins/index.ts`, `src/shell/builtins/cd.ts`, `pwd.ts`, `echo.ts`, `export.ts`, `unset.ts`, `truefalse.ts`
- Test: `src/shell/builtins/builtins.test.ts`, `src/shell/registry.test.ts`

`type` 빌트인은 `builtins/` 안에 두지 않는다. `type` 은 빌트인 표와 coreutils 표를 **모두** 조회해야 하는데, `builtins/index.ts` 가 그것을 import하면 `index → type → index` 순환이 생긴다. 런타임에는 함수 본문에서만 참조하므로 우연히 동작하지만, 순환 import는 모듈 초기화 순서에 의존하는 시한폭탄이다. `type` 은 이미 두 표를 모두 import하는 `registry.ts` 에 둔다.
...
`type` 테스트는 Task 8 시점에 `coreutils` 표가 비어 있으므로 `cat` 케이스가 실패한다. Task 9가 `cat` 을 등록한 뒤 초록불이 된다. Task 8에서는 `cat` 케이스를 `it.todo('coreutil 은 경로로 말한다')` 로 두고, Task 9의 마지막 단계에서 되살린다.
...
- [ ] **Step 6: 빈 coreutils 등록표를 만들어 `registry.ts` 의 import를 만족시킨다**
...
export function commandNames(): string[] {
  return [...Object.keys(builtins), ...Object.keys(coreutils), ...Object.keys(extras)].sort()
}
```
...
**Files:**
- Create: `src/shell/interpreter.ts`, `src/shell/index.ts`, `src/shell/coreutils/shared.ts`, `src/shell/coreutils/cat.ts`
- Modify: `src/shell/coreutils/index.ts`, `src/shell/registry.test.ts` (`it.todo` 되살리기)
...
async function runCommand(node: CommandNode, ctx: RunCtx, stdin: string): Promise<ExecResult> {
  ...
  //    `< file` 로 들어온 stdin 은 크기를 미리 알 수 있다. 파이프는 그렇지 않다.
  //    GNU wc 가 이 차이로 출력 폭을 바꾸므로 명령에게 알려준다.
  let input = stdin
...
- [ ] **Step 5: 코어유틸 공용 헬퍼와 `cat` 을 만든다**

인터프리터 테스트는 `cat`을 쓴다. 임시 스텁을 만들었다가 Task 10에서 버리지 않는다 — 버려질 코드를 커밋하고 리뷰받는 것은 낭비다. 처음부터 최종본을 만든다. 이 헬퍼들은 Task 10·11의 코어유틸 전부가 공유한다.
...
export const cat: CommandFn = (e) => {
  ...
  let stdout = ''
  if (flags.has('n')) {
    // GNU cat -n 은 6칸 우측정렬 + 탭. 파일이 여러 개여도 번호는 이어진다.
    let lineNumber = 1
...
- [ ] **Step 7: Task 8에서 미뤄둔 `type` 테스트를 되살린다**

`src/shell/registry.test.ts` 의 `it.todo('coreutil 은 경로로 말한다')` 를 실제 테스트로 되돌린다.
...
```bash
git add src/shell/interpreter.ts src/shell/index.ts src/shell/interpreter.test.ts src/shell/coreutils src/shell/registry.test.ts
...
`shared.ts` 와 `cat.ts` 는 Task 9 에서 이미 만들어졌다. 여기서는 쓰기만 한다.
...
import type { CommandFn } from '../types'
import { parseFlags, readSources, toLines, byteLength } from './shared'
...
export const wc: CommandFn = (e) => {
  ...
  const rows = sources.map((source) => ({
    label: source.label,
    lines: toLines(source.text).length,
   ...
   *   카운터 1개 + 입력 1개  → 폭 1 (패딩 없음)      `wc -l f`   → "3 f"
   *   그 외 + 크기를 앎      → 폭 = 자릿수(총 바이트)  `wc f`      → " 3  3 14 f"
   *   그 외 + 크기를 모름    → 폭 7                   `cat f | wc` → "      3 …"
   */
  const width =
    counterCount === 1 && rows.length <= 1
      ? 1
      : sizeKnown
...
export const sort: CommandFn = (e) => {
  ...
  lines.sort((a, b) => (flags.has('n') ? Number(a) - Number(b) || byteOrder(a, b) : byteOrder(a, b)))
...
`sort` 어디에도 `localeCompare`를 쓰지 않는다. `localeCompare`는 로케일에 따라 `A`와 `a`의 순서를 뒤집는다. 골든 테스트가 도는 데비안 컨테이너는 `LANG` 이 비어 있어 `C` 로케일이고, 그래서 `sort` 는 바이트 순으로 정렬한다 — 대문자가 먼저 온다(`Apple` < `Hello` < `WORLD` < `apple` < `hello`, 실측). JS의 `<` 연산자가 UTF-16 코드유닛 순이므로 ASCII 범위에서 이 동작과 일치한다. `-n` 의 동점 처리에도 같은 비교를 쓴다.
...
Run: `npx vitest run --project shell src/shell/coreutils/query.test.ts`
Expected: PASS — 전부 초록. `wc` 기대값은 GNU 실측치이므로 하나라도 빨간불이면 폭 규칙 구현을 다시 읽어라.
...
```bash
# tests/shell/golden/cases/10-wc.sh
wc -l a.txt
wc -c a.txt
wc a.txt
wc -lc a.txt
...
cat a.txt | wc -l
wc -l a.txt b.txt
wc fruit.txt nums.txt
```

케이스 09가 성립하는 이유: 이 네 메시지는 GNU coreutils가 프로그램 이름만 접두사로 붙이고 줄번호를 붙이지 않기 때문이다.

케이스 10이 이 계획에서 가장 값진 케이스다. `wc` 의 출력 폭 규칙은 직관에 반한다 — 카운터가 하나면 패딩이 없고, 여럿이면 총 바이트의 자릿수만큼 우측정렬하며, 파이프에서 읽으면 크기를 모르므로 폭이 7로 고정된다. 이 열 줄이 그 규칙 전체를 못박는다.
...
Run: `npm run golden`
Expected: `tests/shell/golden/expected/` 에 10개 `.txt` 파일이 생긴다. 내용을 눈으로 훑어 상식적인지 본다.
...
- `sort` 의 대소문자 순서 → `LC_ALL` 문제. 도커의 데비안은 `LANG` 이 비어 있어 `C` 로케일이므로 바이트 순이다. 우리 `<` 비교와 일치한다.
- `wc` 의 폭 → 파이프(`cat f | wc`)는 폭 7, `< f` 는 파일 크기의 자릿수. `stdinFromFile` 이 제대로 전달되는지 확인하라.
...
- `npm test` — 셸 단위 테스트, 골든 테스트 10개, 문제 테스트 84개, UI 테스트 전부 통과
