...
import { expandWord, expandToSingle, expandForCase, ifsChars, type ExpandCtx } from './expand'
...
describe('expandWord — ${@:o:l}/${*:o:l} 위치 매개변수 슬라이스 (M3 Part 4 task 4 B4, docker debian:stable-slim bash 5 로 확인됨)', () => {
  // docker: f(){ echo "${@:2:2}"; }; f p q r s => q r (문자열 슬라이스였다면 " r" 류로 틀렸을 것)
  it('${@:2:2} 는 원소 리스트 슬라이스(offset2 len2) → q r', async () => {
    ctx.positional = ['p', 'q', 'r', 's']
    expect(await expandWord(wordOf('${@:2:2}'), ctx)).toEqual(['q', 'r'])
  })
  // docker: f(){ echo "${@:1}"; }; f a b c => a b c
  it('${@:1} 은 length 생략 → offset부터 끝까지', async () => {
    ctx.positional = ['a', 'b', 'c']
    expect(await expandWord(wordOf('${@:1}'), ctx)).toEqual(['a', 'b', 'c'])
  })
  // docker: f(){ echo "${@:2}"; }; f a b c => b c
  it('${@:2} 는 offset2부터 끝까지', async () => {
    ctx.positional = ['a', 'b', 'c']
    expect(await expandWord(wordOf('${@:2}'), ctx)).toEqual(['b', 'c'])
  })
  it('${#@} 는 슬라이스 경로와 무관 — 항상 정확한 개수', async () => {
    ctx.positional = ['a', 'b', 'c', 'd']
    expect(await expandWord(wordOf('${#@}'), ctx)).toEqual(['4'])
  })
  // docker: f(){ printf '[%s]' "${@:1:2}"; }; f a b c => [a][b] (엔진엔 printf 없어 per-arg 필드로 확인)
  it('"${@:1:2}" 는 quoted per-arg — 각 원소가 개별 필드(공백 있어도 안 쪼개짐)', async () => {
    ctx.positional = ['a b', 'c', 'd']
    expect(await expandWord(wordOf('"${@:1:2}"'), ctx)).toEqual(['a b', 'c'])
  })
  it('비따옴표 ${@:1:2} 는 조인 후 단어분할 (per-arg 아님)', async () => {
    ctx.positional = ['a b', 'c', 'd']
    expect(await expandWord(wordOf('${@:1:2}'), ctx)).toEqual(['a', 'b', 'c'])
  })
  // docker: f(){ echo "${*:1:2}"; }; f a b c => a b (한 필드로 조인)
  it('${*:1:2} 는 슬라이스 후 IFS 조인 — 항상 한 필드', async () => {
    ctx.positional = ['a', 'b', 'c']
    expect(await expandWord(wordOf('"${*:1:2}"'), ctx)).toEqual(['a b'])
  })
  // $0 오프셋 규약: bash 는 offset 0 을 $0 자리로 본다(offset 1 = $1). 이 엔진은 $0 을
  // 지금은 항상 빈 문자열로 단순화하지만(Task 7/8/9 영역, positionalAt(ctx,0) 참고),
  // 슬라이스의 오프셋 규약 자체는 맞춘다 — extended 리스트의 첫 칸이 $0 이다.
  // docker(실제 $0="bash"): f(){ echo "${@:0:2}"; }; f p q r s => "bash p"
  it('offset 0 은 $0 슬롯을 포함한다(이 엔진 $0 은 항상 "") — "${@:0:2}" → "" p', async () => {
    ctx.positional = ['p', 'q', 'r', 's']
    expect(await expandWord(wordOf('"${@:0:2}"'), ctx)).toEqual(['', 'p'])
  })
  // docker: f(){ echo "${@: -2}"; }; f a b c d => c d ($0 포함 길이 기준 끝에서부터)
  it('음수 offset은 $0 포함 길이 기준 끝에서부터', async () => {
    ctx.positional = ['a', 'b', 'c', 'd']
    expect(await expandWord(wordOf('${@: -2}'), ctx)).toEqual(['c', 'd'])
  })
  it('${@:-fallback} 은 슬라이스가 아니라 기본값 연산자(opChar가 "-") — 위치인자가 있으면 그대로', async () => {
    ctx.positional = ['a', 'b']
    expect(await expandWord(wordOf('${@:-fallback}'), ctx)).toEqual(['a', 'b'])
  })
  it('회귀: 배열 ${arr[@]:1:2} 는 여전히 원소 슬라이스(이 태스크가 안 건드림)', async () => {
    ctx.arrays.set('arr', ['a', 'b', 'c'])
    expect(await expandWord(wordOf('${arr[@]:1:2}'), ctx)).toEqual(['b', 'c'])
  })
  it('회귀: 스칼라 ${X:o:l} 는 여전히 문자열 슬라이스 (X="a b")', async () => {
    expect(await expandWord(wordOf('${X:2:2}'), ctx)).toEqual(['b'])
  })
})
...
describe('ifsChars (M3 Part 4 task 4 B7 — expand.ts/read.ts 공유 헬퍼)', () => {
  // read.ts 가 자체 사본을 두지 않고 이 함수를 그대로 import 해 쓴다(B7). env(Record<string,
  // string>)를 받는 시그니처라 ExpandCtx 없이도 read.ts 에서 바로 호출 가능하다.
  it('IFS 미설정 → 기본 공백류 [space, tab, newline]', () => {
    expect(ifsChars({})).toEqual([' ', '\t', '\n'])
  })
  it('IFS="" (빈 문자열) → 분할 문자 없음', () => {
    expect(ifsChars({ IFS: '' })).toEqual([])
  })
  it('IFS 가 문자열이면 서로 다른 문자들(중복 제거, 등장 순서 보존)', () => {
    expect(ifsChars({ IFS: 'aab' })).toEqual(['a', 'b'])
    expect(ifsChars({ IFS: ' :' })).toEqual([' ', ':'])
  })
})
